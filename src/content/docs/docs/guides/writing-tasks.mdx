...
As an alternative to defining task actions in separate files, you can define them directly inline using `setInlineAction`. This is convenient for simple tasks where creating a separate file would be overkill.
...
You should use `setAction` when you are building a plugin or a complex tasks with a large amount of code, or if it needs to import dependencies. This makes the code more organized, improves the load time of Hardhat (as they are loaded on demand), and makes your setup or plugin more robust to installation errors.

On the other hand, if you are building a simple task that only uses the Hardhat Runtime Environment, you can use `setInlineAction` to define the task's behavior inline, without the boilerplate of a separate file.
