...
You can pass an options object as a second argument to `configVariable`. Its `format` field is a template string where all instances of `{variable}` get replaced with the actual config variable value.
...
You can also pass a `default` value in the options object. Hardhat uses it as a fallback when the Configuration Variable isn't set anywhere else, so resolution won't fail even if you never define the variable:
...
Any value from the environment or a plugin like `hardhat-keystore` takes precedence over the `default`.
