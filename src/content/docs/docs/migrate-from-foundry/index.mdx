...
3.  **Split your profiles across the two axes**

    Foundry has a single profile axis: a `[profile.ci]` section holds compiler settings and test settings together. Hardhat splits them in two, so each Foundry profile becomes up to two Hardhat profiles:
    ...
    Hardhat profiles don't inherit from each other. In Foundry, every profile is merged over `[profile.default]`, so a `[profile.ci]` that only sets `optimizer_runs` keeps everything else from `default`. In Hardhat, each profile is resolved on its own, so you have to copy shared settings into every profile, or share them through a variable in your config file.
...
3.  **Port your test execution settings**
    ...
    If you have more than one `foundry.toml` profile, port each one's test settings into [Test Profiles](/docs/guides/testing/test-profiles) under `test.solidity.profiles`, and select them with `--test-profile`. Remember that Hardhat profiles don't inherit from `default`, so include shared settings in each profile, either directly or through a shared variable:
...
4.  **Convert your `forge-config:` annotations**

    Hardhat accepts the `forge-config:` prefix, so [inline configuration](/docs/guides/testing/inline-configuration) keeps working. But the meaning of a `default.` prefix changes, so it needs converting:
    ...
    In Foundry, `default.` applies under every profile, because profiles inherit from `default`. In Hardhat it applies only under the `default` profile. Dropping the prefix keeps the Foundry behavior of applying everywhere.

    :::caution

    This one fails silently. If you leave the `default.` prefixes in place, your tests keep passing and the annotations keep working under the `default` profile. Under any other Test Profile, every `default.`-prefixed annotation stops applying, with no error.

    :::

    Annotations for other profiles, like `ci.fuzz.runs`, keep their meaning and need no change. The profile must be declared in `test.solidity.profiles`. Otherwise, the run fails.

5.  **Enable `ffi` and filesystem access if your tests need them**
...
6.  **Replace `anvil` with the in-process network (or `hardhat node`)**
