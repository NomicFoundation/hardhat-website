...
Note that a `default.` prefix means "only under the `default` profile" in Hardhat, while in Foundry it means "under every profile". If you're migrating annotations from Foundry, read the [Foundry migration guide](/docs/migrate-from-foundry).
