...
- **Maintainability**: Separating task logic into dedicated files improves code organization
...
Inline actions are loaded immediately when the configuration file is evaluated, so they don't benefit from lazy loading.
...
Each task must define exactly one action:

- You must call either `setAction()` or `setInlineAction()`, but not both
- You cannot call the same method multiple times on the same task
