...
`slang-solx` provides Yul optimization via `settings.optimizer.enabled`, which is `false` by default. Enabling it optimizes on top of `LLVM` rather than replacing it:
